...
| LLM providers: Ollama, or any OpenAI-compatible endpoint (LiteLLM at work) | `rata-llm-providers`, tracked default `http://localhost:11434`; the work proxy is set in `local.el` | `init-llm.el` (`gptel`, `ellama`, `aidermacs`, all derived from the one list — D-022) | Ollama: none. `openai` entries: `~/.authinfo.gpg`, `machine <url host> login apikey`, read at request time (gptel/llm key closure) or at spawn (aider, inside aidermacs's `process-environment` let) | Tracked default models `qwen3.5-coder:9b-32k`, `mistral:latest`, embeddings `nomic-embed-text`. A malformed entry is a startup warning (`just batch-strict` fails). Buffer contents go wherever the *first* entry points — on a work machine that is the corporate proxy, not the machine |
| Khoj (self-hosted) | `rata-khoj-server-url`, default `http://khoj.homelab.local`, overridable in `local.el` (L-051) | `init-khoj.el` | none configured | **indexes `~/workspace/second-brain/org-roam/`** — sends the operator's notes to the homelab host |
...
| Proton Mail, via Proton Mail Bridge | `127.0.0.1:1143` IMAP and `127.0.0.1:1025` SMTP, both STARTTLS behind Bridge's self-signed cert | `init-mail.el` (mu4e reads the Maildir `mbsync` fills; `smtpmail` sends) | `~/.authinfo.gpg`, `machine 127.0.0.1 port 1143/1025 login <address>` — the Bridge password, not the Proton one | Bridge is the Flatpak `ch.protonmail.protonmail-bridge` on the Ubuntu host (3.26.0, installed 2026-09-11) and `extra/protonmail-bridge` on Arch (3.26.0, 2026-09-12); it needs a **paid plan** and a keychain — Secret Service (gnome-keyring) on Ubuntu, `pass` on the Arch/Hyprland host. On Arch `protonmail-bridge` on PATH is a Qt launcher that cannot drive `--cli`; use `/usr/lib/protonmail/bridge/bridge --cli` / `--noninteractive` (FAIL-0018). Bridge 3.x writes no `cert.pem`: `cert export` in the CLI, to `~/.config/protonmail/bridge-v3` (native) or the Flatpak sandbox path. `mu` is AUR-only on Arch (1.14.3). `mu` 1.14.3 and `isync` 1.5.1 from Homebrew — apt's are too old (D-021). The address is `rata-mail-address` in `local.el`, nil in git. `mu4e` is loaded from beside the `mu` binary, not from elpaca, because the elisp is version-locked to the binary. `~/.mbsyncrc` is per-machine, from `mbsyncrc.example`. `SPC a e d` reports what is missing on a host |
