...
| [FAIL-0018](failures/FAIL-0018.md) | LOW | environment-drift, documentation | `init-mail.el`, `mbsyncrc.example`, `just/deps-arch.just` | Mail setup hints were Flatpak literals: on Arch `protonmail-bridge --cli` hangs (launcher, not Bridge), and no `cert.pem` exists until `cert export` | introduced 2026-09-11 with the module | FIXED | `rata-test-mail-bridge-command-prefers-the-native-binary`, `rata-test-mail-cert-dir-matches-template` |
| [FAIL-0019](failures/FAIL-0019.md) | MEDIUM | environment-drift, verification-gap | `init-llm.el`, `tests/run-tests.el`, `elpaca/sources/agent-shell/` | The agent-shell Enter-key fix (`3492fb0`) extends `agent-shell-ui-fragment-map`, added upstream 2026-08-14; both clones on this machine are from March, so the `eval-after-load` body signalled `void-variable` on the first agent shell of a session and its two tests had never passed here — no lockfile, so each machine's clone is its own vintage | introduced by `3492fb0`, latent here from day one | FIXED — guarded; tests skip naming the remedy; agent-shell + shell-maker + acp updated by the operator 2026-09-13 and both tests pass. The off-chrome test now asserts the leak invariant, not one evil-collection vintage's theme | the `ert-skip` reason; no gate short of a lockfile (D-005) |
| [FAIL-0020](failures/FAIL-0020.md) | MEDIUM | verification-gap, self-inflicted | `just/emacs.just` `batch`, `tests/run-tests.el` | `--batch` skips init, so `after-init-time` was already set when `-l init.el` ran and elpaca took its post-init path: with `elpaca-queue-limit 5` a queue finalised while its last throttled orders were still queued, so `use-package` bodies saw `nerd-icons-corfu` unloadable and `transient` came from Emacs's built-in copy — warnings a real startup never produces, misread as FAIL-0012 and then as FAIL-0015 | pre-existing in the harness | FIXED — the harness clears `after-init-time` before loading | `just batch-strict`, now on the init path |
| [FAIL-0021](failures/FAIL-0021.md) | HIGH | verification-gap, safety | `tests/run-tests.el` | `rata-test-jira-mirrored-keys-exist-upstream` turned on `jira-tempo-mode`, which reverts its table on activation — a live Tempo request that decrypts `~/.authinfo.gpg`: a passphrase prompt and exit 255 before ERT's summary in batch, a silent call to the corporate instance when gpg-agent held the key. Every ERT run since 2026-09-08 had ended this way | introduced by `593469f` | FIXED — refresh stubbed in the test; `jira-api-call` and `jira-api-tempo-call` overridden harness-wide to fail loudly | the two overrides in `tests/run-tests.el` (verified by driving `jira-tempo-mode` against them) |
| [FAIL-0022](failures/FAIL-0022.md) | MEDIUM | verification-gap, self-inflicted | `tests/run-tests.el`, `just/test.just` | A new test did `(require 'aidermacs)`; its vterm backend requires vterm, which asks "Compile vterm-module? (y or n)" at load — a blocking stdin read in batch, so the suite hung at test 83 of 109 with no failure and no summary until killed | introduced 2026-09-14 with the D-022 tests | FIXED — test requires `aidermacs-models` only; `read-from-minibuffer`, `read-string` and `yes-or-no-p` overridden harness-wide to fail loudly (probed: all five batch prompt kinds signal); `test-ert` runs with stdin from `/dev/null` | the `rata-test-no-stdin` advice in `tests/run-tests.el`; `< /dev/null` in `just/test.just` |
