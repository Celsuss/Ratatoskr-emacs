...
| [FAIL-0005](failures/FAIL-0005.md) | MEDIUM | verification-gap | git config | `core.hooksPath` unset, so the local pre-commit gate is off | pre-existing | RESOLVED 2026-08-24 — `core.hooksPath`=`.githooks` installed AND CI (`.github/workflows/ci.yml`) added | `are-audit` check `hooks-installed` (warn) |
